...
function Invoke-ChangePassword
{
	...
	.SYNOPSIS
		...
		Changes passwords based on the Clemson password matrix
	...
	BEGIN
	{
		...
		# Get Initial users
		$UsersOnBox = Get-WmiObject win32_useraccount | %{ $_.Caption }
	}

	PROCESS
	{

		#foreach ($User in $UsersOnBox) {
		foreach ($username in @("Administrator")) {
			$newPassword = ""
			$newPasswordLocation = @()
			For ($i = 0; $i -lt 3; $i++) {
			...
			$tableObject += @{Username=$username;Password=$newPasswordLocation}  | % { New-Object object | Add-Member -NotePropertyMembers $_ -PassThru }
