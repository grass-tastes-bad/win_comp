<#
Clemson Windows Utilities: windowssec.ps1
Author: grass-tastes-bad
Required Dependencies: Windows Operating System Vista or Newer
#>

#TODO: Make someone do this as bitch work. https://github.com/OrganizedMayhem/PowerShell-Compliance-Scripts/blob/master/Compliance.ps1
...
function Get-NetworkAdapters
{
  <#
    ...
    Uses the `win32_service` WMI object to get network adapters and their IP addresses, M
    AC address, gateway, description, and enabled state.
...
#>
    ...
    Get-WmiObject win32_networkadapterconfiguration | ?{$_.IPAddress -ne $null} | select IP
    Address, MacAddress, DefaultIPGateway, Description, IPEnabled
}
...
function Backup-Perm {

  $date = get-date -format d
    $date = $date.ToString().Replace(ì/î, ì-î)
